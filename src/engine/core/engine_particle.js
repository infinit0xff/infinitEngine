...
infinitEngine.Particle = (function () {
    var ivSystemtAcceleration = [0, -50.0];
    ...
    var resolveCirclePos = function (circShape, particle) {
        ...
        vec2.subtract(ivFrom1to2, pos, cPos);
        var dist = vec2.length(ivFrom1to2);
        if (dist < circShape.getRadius()) {
            vec2.scale(ivFrom1to2, ivFrom1to2, 1/dist);
            vec2.scaleAndAdd(pos, cPos, ivFrom1to2, circShape.getRadius());
            collided = true;
        }
        return collided;
    };
    ...
    var resolveRectPos = function (rectShape, particle) {
        ...
        if (collided) {
            vec2.subtract(ivFrom1to2, pos, rPos);
            ivVec[0] = ivFrom1to2[0];
            ivVec[1] = ivFrom1to2[1];

            // find closest axis
            if (Math.abs(ivFrom1to2[0] - alongX) < Math.abs(ivFrom1to2[1] - alongY))  {
            ...
            vec2.subtract(ivVec, ivVec, ivFrom1to2);
            vec2.add(pos, pos, ivVec);  // remember pos is particle position
        }
        return collided;
    };
