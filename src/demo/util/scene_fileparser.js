function SceneFileParser(sceneFilePath) {
    this.ivSceneXml = infinitEngine.ResourceMap.retrieveAsset(sceneFilePath);
}

SceneFileParser.prototype._getElm = function(tagElm) {
    var theElm = this.ivSceneXml.getElementsByTagName(tagElm);
    if (theElm.length === 0)
...
SceneFileParser.prototype.parseSquares = function(sqSet) {
    ...
    for (i=0; i<elm.length; i++) {
        ...
        sq = new Renderable(infinitEngine.DefaultResources.getConstColorShader());
